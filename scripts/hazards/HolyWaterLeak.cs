...
public partial class HolyWaterLeak : Node3D, ICatchArea, IRitualInteract, IHazard
{
    [Export]
    public WorkSlot Slot;

    [Export]
    public GpuParticles3D Particles;

    [Export]
    public float DisruptionRadius = 10f;
    ...
    public void SetDisruptStatus(bool disrupting)
    {
        Particles.Emitting = disrupting;
        GD.Print($"Stall at position {GlobalPosition}");
        EventBus.Publish(
            new SetDisruptFacilityInRange(GlobalPosition, DisruptionRadius, disrupting)
        );
    }
